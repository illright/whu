...
<div class="grid grid-cols-[auto,1fr] gap-x-4 gap-y-0.5">
  <Switch.Root
    class={twJoin(
  ...
  <label class="text-sm font-semibold" for={rootName} use:asLabel>{label}</label>
  {#if description !== null}
    <p class="text-xs text-neutral-500 col-start-2">{description}</p>
  {/if}
</div>
